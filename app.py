...
def post_process(text, silence_duration=0.3):
    ...
    independent_phrases = [
        ...
        r'\bdạ\s+(cậu|anh|chị|má|ba|ông|bà)',  # "dạ cậu ba", "dạ má"
    ...
    def smart_comma_split(sentence):
        ...
        for i in range(1, len(parts)):
            ...
            # Kiểm tra nếu phần tiếp theo là câu độc lập
    ...
    for sent in sentences_by_period:
        ...
        # Áp dụng logic tách dấu phẩy thông minh
        sub_sentences = smart_comma_split(sent)
        all_sentences.extend(sub_sentences)
    
    # Loại bỏ ký tự đặc biệt (trừ dấu phẩy và placeholder)
    ...
    for sentence in all_sentences:
        ...
        # Chỉ loại bỏ ký tự đặc biệt nếu không có placeholder
...
demo.queue().launch(share=True),
    ...
    def smart_comma_split(sentence):
        ...
        for i in range(1, len(parts)):
            ...
            # 3. Kiểm tra nếu phần tiếp theo quá ngắn (< 15 ký tự) và không có ngữ cảnh
            if len(next_part) < 15 and not any(word in next_part for word in ['của', 'và', 'với', 'cho', 'bởi']):
                is_independent = True
...
demo.queue().launch(share=True),
        ...
        # 14. Pattern: câu quá ngắn (< 10 ký tự) không có từ liên kết
        # Sẽ xử lý trong logic bên dưới
    ]
