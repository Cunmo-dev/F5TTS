...
def split_text_into_sentences(text, pause_paragraph_duration=0.8, pause_dialogue_duration=0.4):
    ...
    for line in lines:
        ...
        # Bỏ qua các dòng chỉ chứa ký tự đặc biệt (như +, -, *, =, etc)
        if re.match(r'^[\+\-\*=_#@&%$!~`|\\\/]+
...
demo.queue().launch(share=True), line):
            continue
        ...
        # Loại bỏ dấu ngoặc kép và ký tự + để xử lý
        clean_text = line.replace('"', '').replace('"', '').replace('"', '').replace('+', '').strip()
        
        # Bỏ qua nếu sau khi làm sạch không còn gì
        if not clean_text:
            continue
